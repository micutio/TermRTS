...
{
    private readonly int _cellCount = cellCount;
    ...
    public byte[,] Generate(int worldWidth, int worldHeight)
    {
        ...
        var voronois = new int[_cellCount];
        for (var i = 0; i < _cellCount; i += 1) voronois[i] = _rng.Next(worldWidth * worldHeight);

        var landWaterMap = _rng.GetItems([3, 4], _cellCount);
